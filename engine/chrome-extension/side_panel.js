...
function updateStatus(data) {
  ...
  const pointsEl = document.getElementById('flow-points');
  const points = Number(data.remainingCredits);
  const hasPoints = Number.isFinite(points) && points >= 0 && data.remainingCredits !== null;
  const estimatedVideos = hasPoints ? Math.floor(points / 15) : null;
  pointsEl.textContent = hasPoints
    ? `Sisa Point ${points.toLocaleString('id-ID')} / ±${estimatedVideos.toLocaleString('id-ID')} Video`
    : 'Sisa Point — / ±— Video';
  pointsEl.classList.toggle('has-value', hasPoints);
...
document.addEventListener('DOMContentLoaded', () => {
  fetchStatus();
  fetchLog();
  sendMessage({ type: 'REFRESH_CREDITS' }).then(fetchStatus).catch(() => {});
  startTickers();
});
